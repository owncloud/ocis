...
`UPDATE_PASSWORD,VERIFY_EMAIL`; unknown actions are ignored and the service falls
back to the defaults if no valid action remains, so an invited guest always has a
way to set up their account.
