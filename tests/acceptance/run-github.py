...
Config is sourced from .drone.star localApiTests — single source of truth.
...
# ---------------------------------------------------------------------------
# Reverse lookup: suite name → group config
# ---------------------------------------------------------------------------

_SUITE_TO_CONFIG: dict = {}
for _group_config in LOCAL_API_TESTS.values():
    for _suite in _group_config.get("suites", []):
        _SUITE_TO_CONFIG[_suite] = _group_config


def suite_config(suite: str) -> dict:
    """Return merged config for a single suite, or empty dict if not found."""
    return _SUITE_TO_CONFIG.get(suite, {})


def merged_config(suites: list[str]) -> dict:
    """Merge config for all requested suites (union of extra envs, OR of flags)."""
    merged: dict = {
        "emailNeeded": False,
...
# ---------------------------------------------------------------------------
# Base ocis server environment (from drone ocisServer() function)
# ---------------------------------------------------------------------------
...
def run(cmd: list[str], env: dict = None, check: bool = True) -> subprocess.CompletedProcess:
    e = {**os.environ, **(env or {})}
...
def main() -> int:
    ...
    print(f"Config: emailNeeded={cfg['emailNeeded']} antivirusNeeded={cfg['antivirusNeeded']} "
          f"tikaNeeded={cfg['tikaNeeded']}")
    ...
    # --- php deps ---
    run(["composer", "install", "--no-progress"], env={"COMPOSER_NO_INTERACTION": "1"})
    ...
    # --- build ociswrapper env ---
    server_env = {**os.environ}
    server_env.update(base_server_env(repo_root, ocis_url, str(ocis_config_dir)))
    server_env.update(cfg["extraServerEnvironment"])
    # convert bool values to strings (from drone.star Starlark booleans)
    server_env = {k: str(v).lower() if isinstance(v, bool) else str(v)
                  for k, v in server_env.items()}

    # --- start ociswrapper ---
    print("Starting ocis via ociswrapper...")
    ...
    try:
        # --- wait for ocis ---
        print("Waiting for ocis...")
        wait_for(lambda: ocis_healthy(ocis_url), 300, "ocis")
        print("ocis ready.")

        # --- prepare expected failures (drone merges without-remotephp.md) ---
        ...
        expected_failures_override = os.environ.get("EXPECTED_FAILURES_FILE")
        if expected_failures_override:
            base_failures = Path(expected_failures_override)

        # merge without-remotephp entries (matches drone localApiTests command)
        tmp_failures = tempfile.NamedTemporaryFile(
            mode="w", suffix=".md", delete=False, dir="/tmp"
        )
        tmp_failures.write(base_failures.read_text())
        without_remotephp = repo_root / "tests/acceptance/expected-failures-without-remotephp.md"
        if without_remotephp.exists():
            tmp_failures.write("\n")
            tmp_failures.write(without_remotephp.read_text())
        tmp_failures.close()

        # required by drone before running behat
        Path("/etc/ocis").mkdir(parents=True, exist_ok=True)
        ...
        behat_env = {
            **os.environ,
            ...
            "EXPECTED_FAILURES_FILE": tmp_failures.name,
            "STORAGE_DRIVER": "ocis",
