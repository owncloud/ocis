Feature: State of the quota
  ...
  Scenario: upload a file by setting OCIS spaces max quota
    Given the config "STORAGE_USERS_OCIS_MAX_QUOTA" has been set to "10" for "storage-users" service
  ...
  Scenario: try to create a space with quota greater than OCIS spaces max quota
    Given the config "STORAGE_USERS_OCIS_MAX_QUOTA" has been set to "50" for "storage-users" service
    And user "Brian" has been created with default attributes
