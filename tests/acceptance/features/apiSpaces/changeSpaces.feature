...
Feature: Change data of space
  ...
  Scenario Outline: Only space admin user can change the name of a Space via the Graph API
    ...
    Then the json responded should contain a space "<expectedName>" with these key and value pairs:
      | key       | value          |
      | driveType | project        |
      | name      | <expectedName> |
    Examples:
  ...
  Scenario: An user space admin set readme file as description of the space via the Graph API
    ...
    Then the HTTP status code should be "200"
    When user "Alice" lists all available spaces via the GraphApi
    Then the json responded should contain a space "Project Jupiter" owned by "Alice" with description file ".space/readme.md" with these key and value pairs:
      | key                                | value           |
      | name                               | Project Jupiter |
      | special@@@0@@@size                 | 17              |
  ...
  Scenario Outline: An user space admin and editor set image file as space image of the space via the Graph API
    ...
    Then the HTTP status code should be "200"
    When user "Alice" lists all available spaces via the GraphApi
    Then the json responded should contain a space "Project Jupiter" owned by "Alice" with description file ".space/<fileName>" with these key and value pairs:
      | key                                | value            |
      | name                               | Project Jupiter  |
      | special@@@0@@@size                 | 0                |
  ...
  Scenario Outline: An user set new image file as space image of the space via the Graph API
    ...
    Then the HTTP status code should be "200"
    When user "<user>" lists all available spaces via the GraphApi
    Then the json responded should contain a space "Project Jupiter" owned by "Alice" with description file ".space/newSpaceImage.png" with these key and value pairs:
      | key                                | value             |
      | name                               | Project Jupiter   |
      | special@@@0@@@size                 | 0                 |
