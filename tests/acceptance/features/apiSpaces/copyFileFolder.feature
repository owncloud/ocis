...
Feature: check etag propagation after copying files or folders
  ...
  Scenario: as share receiver copying a file inside a folder changes its etag for all collaborators
    ...
    And user "Alice" has stored etag of element "/" in space "Personal"
    And user "Alice" has stored etag of element "/upload" in space "Personal"
    And user "Alice" has stored etag of element "/upload/file.txt" in space "Personal"
    And user "Alice" has stored etag of element "/upload/file.txt" on path "/upload/renamed.txt" in space "Personal"
    And user "Brian" has stored etag of element "/" in space "Shares Jail
    And user "Brian" has stored etag of element "/upload" in space "Shares Jail"
    And user "Brian" has stored etag of element "/upload/file.txt" in space "Shares Jail"
    And user "Brian" has stored etag of element "upload/file.txt" on path "upload/renamed.txt" in space "Shares Jail"
