Feature: collaboration (wopi)
  ...
  Scenario Outline: open file with .odt extension (open-with-web)
    ...
    Examples:
      ...
      | /app/open-with-web?file_id=<<FILEID>>                     | app=(Collabora\|OnlyOffice\|FakeOffice)& |
    ...
    And the JSON data of the response should match
      """
      {
        ...
        "properties": {
          "uri": {
            "type": "string",
             "pattern": "%base_url%/external\\?app=(Collabora|OnlyOffice|FakeOffice)&contextRouteName=files-spaces-personal&fileId=%uuidv4_pattern%%24%uuidv4_pattern%%21%uuidv4_pattern%$"
          }
        }
      }
      """
