Feature: check activities
  ...
  Scenario: check rename activity for a file
    Given user "Alice" has been created with default attributes and without skeleton files
    And user "Alice" has uploaded file with content "ownCloud test text file 0" to "/textfile0.txt"
    When user "Alice" renames file "textfile0.txt" to "/renamed.txt" in space "Personal" using the WebDAV API
    And user "Alice" checks the activities for file "renamed.txt" in space "Personal" using the Graph API
    Then the HTTP status code should be "200"
    And the JSON data of the response should match
      """
      {
