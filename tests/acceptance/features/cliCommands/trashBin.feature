...
Feature: trashbin
  ...
  Scenario: purge expired trash resource of Personal space
    Given the following configs have been set:
      | config                                               | value |
      | STORAGE_USERS_PURGE_TRASH_BIN_PERSONAL_DELETE_BEFORE | 5s    |
  ...
  Scenario: purge expired trash resource of project space
    Given the following configs have been set:
      | config                                              | value |
      | STORAGE_USERS_PURGE_TRASH_BIN_PROJECT_DELETE_BEFORE | 5s    |
    And using spaces DAV path
    And the administrator has assigned the role "Space Admin" to user "Alice" using the Graph API
    And user "Alice" has created the following folders
      | path              |
