...
Feature: List upload sessions via CLI command
  ...
  Scenario: list and cleanup the expired upload sessions
    ...
    And user "Alice" has uploaded file "filesForUpload/textfile.txt" to "file.txt" via TUS protocol
    And user "Alice" has tried to upload file "filesForUpload/large.zip" to "large.zip" via TUS protocol
