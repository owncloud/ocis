...
class SharingNgContext implements Context {
	...
	public function checkIfShareExists(string $share, string $sharee, string $sharer): void {
		...
		foreach ($driveList as $drive) {
			if ($drive->driveType === "mountpoint" && $drive->name === $share && $drive->root->remoteItem->driveAlias === "personal/brian") {
	...
	/**
	 * @When user :sharee should have a share :share shared by user :sharer
	 *
	 * @param string $sharee
	 * @param string $share
	 * @param string $sharer
