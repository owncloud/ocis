Feature: resources shared by user
  ...
  Scenario: sharer (also a group member) lists the file and folder shares shared to group (Personal space)
    Given group "grp1" has been created
  ...
  Scenario: sharer (also a group member) lists the file and folder shares shared to group (Project space)
    Given group "grp1" has been created
    And user "Alice" has been added to group "grp1"
    And using spaces DAV path
    And the administrator has assigned the role "Space Admin" to user "Alice" using the Graph API
