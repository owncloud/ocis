Feature: an user gets the resources shared to them
  ...
  Scenario: sharee (also a group member) lists the file and folder shares shared to group (Personal space)
  ...
  Scenario: sharee (also a group member) lists the file and folder shares shared to group (Project space)
    Given group "grp1" has been created
    And user "Alice" has been added to group "grp1"
    And using spaces DAV path
    And the administrator has assigned the role "Space Admin" to user "Alice" using the Graph API
