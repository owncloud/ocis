...
trait WebDav {
	...
	/**
	 * @When user :user creates folder :destination using the WebDAV API with headers
	 *
	 * @param string $user
	 * @param string $destination
	 * @param TableNode $table
	 ...
	 * @throws GuzzleException
	 */
	public function userCreatesFolderWithHeaders(string $user, string $destination, TableNode $table): void {
		$headers = [];
		foreach ($table->getColumnsHash() as $header) {
			$headerName = $header['header'];
