...
class SetupHelper extends \PHPUnit\Framework\Assert {
	...
	public static function runOcc(
		...
		if ($resultXml === false) {
			throw new Exception(
				"Response is not valid XML after executing 'occ " . join(" ", $args) . "'. " .
				$isTestingAppEnabledText .
				$contents
			);
		}
		...
		if (!isset($return['code'][0])) {
			throw new Exception(
				"Return code not found after executing 'occ " . join(" ", $args) . "'. " .
				$isTestingAppEnabledText .
				$contents
			);
		}
		...
		if (!isset($return['stdOut'][0])) {
			throw new Exception(
				"Return stdOut not found after executing 'occ " . join(" ", $args) . "'. " .
				$isTestingAppEnabledText .
				$contents
			);
		}
		...
		if (!isset($return['stdErr'][0])) {
			throw new Exception(
				"Return stdErr not found after executing 'occ '" . join(" ", $args) . "'. " .
				$isTestingAppEnabledText .
				$contents
			);
		}
		...
		if (!\is_a($return['code'][0], "SimpleXMLElement")) {
			throw new Exception(
				"Return code is not a SimpleXMLElement after executing 'occ " . join(" ", $args) . "'. " .
				$isTestingAppEnabledText .
				$contents
			);
		}
		...
		if (!\is_a($return['stdOut'][0], "SimpleXMLElement")) {
			throw new Exception(
				"Return stdOut is not a SimpleXMLElement after executing 'occ " . join(" ", $args) . "'. " .
				$isTestingAppEnabledText .
				$contents
			);
		}
		...
		if (!\is_a($return['stdErr'][0], "SimpleXMLElement")) {
			throw new Exception(
				"Return stdErr is not a SimpleXMLElement after executing 'occ " . join(" ", $args) . "'. " .
				$isTestingAppEnabledText .
				$contents
			);
		}
