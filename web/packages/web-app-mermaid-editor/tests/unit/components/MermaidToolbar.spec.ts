...
describe('MermaidToolbar', () => {
  it('renders the three view-mode buttons', () => {
    ...
    expect(wrapper.find('.mermaid-editor-viewmode-editor').exists()).toBe(true)
    expect(wrapper.find('.mermaid-editor-viewmode-split').exists()).toBe(true)
    expect(wrapper.find('.mermaid-editor-viewmode-preview').exists()).toBe(true)
  })
  ...
  it('emits changeMode with the chosen mode', async () => {
    const { wrapper } = getWrapper('split')
    await wrapper.find('.mermaid-editor-viewmode-preview').trigger('click')
