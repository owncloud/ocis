<template>
  <div class="mermaid-editor-toolbar oc-flex oc-flex-middle oc-px-s oc-py-xs">
    <div class="oc-button-group" role="group" :aria-label="$gettext('View mode')">
      <oc-button
        v-for="mode in modes"
        :key="mode.name"
        v-oc-tooltip="$gettext(mode.label)"
        :class="`mermaid-editor-viewmode-${mode.name}`"
...
export type MermaidEditorViewMode = 'editor' | 'split' | 'preview'

interface Props {
  viewMode: MermaidEditorViewMode
}
interface Emits {
  (e: 'changeMode', value: MermaidEditorViewMode): void
}
defineProps<Props>()
defineEmits<Emits>()

const modes: { name: MermaidEditorViewMode; label: string; icon: string }[] = [
