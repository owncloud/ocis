    ...
    return {
        ...
        "steps": skipIfUnchanged(ctx, "acceptance-tests") + restoreBuildArtifactCache(ctx, "ocis-binary-amd64", "ocis/bin/ocis") +
                 ocisServer(storage, accounts_hash_difficulty, [stepVolumeOC10Tests]) + waitForSeleniumService() + waitForMiddlewareService() + [
            {
                ...
                "commands": [
                    "sudo sh -c 'curl -o /usr/local/bin/retry https://raw.githubusercontent.com/owncloud-ops/retry/master/retry && chmod +x /usr/local/bin/retry'",
                    ". /drone/src/.drone.env",
                    ...
                    "cd tests/acceptance/",
                    "retry -t3 yarn install --immutable",
                    "./run.sh",
                ],
            },
        ] + failEarly(ctx, early_fail),
