...
def injectWopiConfig():
    ...
    return """awk 'BEGIN {ocm_found=0; inserted=0} /^[[:space:]]*ocm:[[:space:]]*$/ {ocm_found=1} ocm_found && /^[[:space:]]*enabled:[[:space:]]*.*$/ && !inserted {print; print "  appsIntegration:"; print "    enabled: true"; print "    wopiIntegration:"; print "      officeSuites:"; print "        - name: FakeOffice"; print "          product: MicrosoftOfficeOnline"; print "          enabled: true"; print "          uri: http://fakeoffice:8080"; print "          insecure: true"; print "          disableProof: true"; print "          disableChat: true"; print "          postMessageOrigin: https://ocis-server"; print "          wopiSrc: http://localhost:9300"; print "          ingress:"; print "            enabled: false"; print "        - name: Collabora"; print "          product: Collabora"; print "          enabled: true"; print "          uri: http://fakeoffice:8080"; print "          insecure: true"; print "          disableProof: true"; print "          postMessageOrigin: https://ocis-server"; print "          wopiSrc: http://localhost:9302"; print "          ingress:"; print "            enabled: false"; print "        - name: OnlyOffice"; print "          product: OnlyOffice"; print "          enabled: true"; print "          uri: http://fakeoffice:8080"; print "          insecure: true"; print "          disableProof: true"; print "          disableChat: true"; print "          postMessageOrigin: https://ocis-server"; print "          wopiSrc: http://localhost:9305"; print "          ingress:"; print "            enabled: false"; print "    mimetypes:"; print "      - mime_type: application/vnd.oasis.opendocument.text"; print "        extension: odt"; print "        name: OpenDocument Text"; print "        default_app: FakeOffice"; print "        allow_creation: true"; print "      - mime_type: text/plain"; print "        extension: txt"; print "        name: Plain Text"; print "        default_app: FakeOffice"; print "        allow_creation: true"; inserted=1; next} {print}' ./charts/ocis/ci/deployment-values.yaml > /tmp/dv.tmp && mv /tmp/dv.tmp ./charts/ocis/ci/deployment-values.yaml && echo "Injected appsIntegration config" """
