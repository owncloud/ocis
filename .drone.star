    ...
    steps = [
        {
            ...
            "commands": [
                ...
                "sh -c 'if command -v protoc >/dev/null 2>&1; then echo protoc present; elif command -v apk >/dev/null 2>&1; then apk add --no-cache protobuf; elif command -v apt-get >/dev/null 2>&1; then apt-get update && apt-get install -y protobuf-compiler; elif command -v yum >/dev/null 2>&1; then yum install -y protobuf-compiler; else echo \"No supported pkg manager found\"; exit 1; fi'",
                "cd tools/protoc-gen-microweb",
                "GOWORK=off go build -o ../../bin/protoc-gen-microweb .",
                "chmod +x test.sh",
                "./test.sh",
            ],
            "volumes": [
