...
Is the pre-migration stage when having a functional ownCloud Classic 10 instance.
...
For more details see: [ownCloud Classic 10 with ownCloud Web](https://owncloud.dev/clients/web/deployments/oc10-app/)
...
#### Validation
Ensure switching back and forth between the ownCloud Classic 10 web UI and ownCloud web works as at our https://demo.owncloud.com.
...
2. Add [Openid Connect (OIDC)](https://doc.owncloud.com/server/admin_manual/configuration/user/oidc/) support to ownCloud Classic 10.
...
While OpenID Connect providers will send an `iss` and `sub` claim that relying parties (services like oCIS or ownCloud Classic 10) can use to identify users we recommend introducing a dedicated, globally unique, persistent, non-reassignable user identifier like a UUID for every user. This `ownclouduuid` should be sent as an additional claim to save additional lookups on the server side. It will become the user id in oCIS, e.g. when searching for recipients the `ownclouduuid` will be used to persist permissions with the share manager. It has a different purpose than the ownCloud Classic 10 username, which is used to login. Using UUIDs we can not only mitigate username collisions when merging multiple instances but also allow renaming usernames after the migration to oCIS has been completed.
...
- blobs from the same datadirectory layout as in ownCloud Classic 10
- metadata from the ownCloud Classic 10 database:
...
_TODO we need a share manager that can read from the ownCloud Classic 10 database as well as from whatever new backend will be used for a pure oCIS setup. Currently, that would be the json file. Or that is migrated after all users have switched to oCIS. -- jfd_
...
We are going to run and explore a series of services that will together handle the same requests as ownCloud Classic 10. For initial exploration the oCIS binary is recommended. The services can later be deployed using a single oCIS runtime or in multiple containers.
...
Test writing data with oCIS into the existing ownCloud Classic 10 data directory using the `owncloudsql` storage driver.
...
In the previous stages oCIS was only accessible for administrators with access to the network. To expose only a single service to the internet, oCIS comes with a user aware proxy that can be used to route requests to the existing ownCloud Classic 10 installation or oCIS, based on the authenticated user. The proxy uses OIDC to identify the logged-in user and route them to the configured backend.

#### User impact
The IP address of the ownCloud host changes. There is no change for the file sync and share functionality when requests are handled by the oCIS codebase as it uses the same database and storage system as ownCloud Classic 10.

#### Steps and verifications

##### Deploy oCIS proxy
1. Deploy the `ocis proxy`
2. Verify the requests are routed based on the ownCloud Classic 10 routing policy `oc10` by default
...
#### Rollback
Should there be a problem with the oCIS routes the user can be routed to ownCloud by changing his routing policy. In case of unfixable problems with the proxy the DNS needs to be updated to use the ownCloud Classic 10 application servers directly. This could also be done in a load balancer.
...
#### Rollback
Until now, the oCIS configuration mimics ownCloud Classic 10 and uses the old data directory layout and the ownCloud Classic 10 database. Users can seamlessly be switched from ownCloud Classic 10 to oCIS and back again.
<div class="editpage">
...
To encourage users to switch you can promote the workspaces feature that is built into oCIS. The ownCloud Classic 10 storage backend can be used for existing users. New users and group or project spaces can be provided by storage providers that better suit the underlying storage system.
...
### Stage-8: shut down ownCloud Classic 10
Disable ownCloud Classic 10 in the proxy, all requests are now handled by oCIS, shut down oc10 web servers and redis (or keep for calendar & contacts only? rip out files from oCIS?)

#### User impact
All users are already sent to the oCIS backend. Shutting down ownCloud Classic 10 will remove the old web UI, apps and functionality that is not yet present in ownCloud web. For example contacts and calendar.
...
#### Steps
1. Shut down the apache servers that are running the ownCloud Classic 10 PHP code.
2. DO NOT SHUT DOWN THE DATABASE, YET!

#### Verification
The ownCloud Classic 10 web UI should no longer be reachable.

#### Rollback
Redeploy ownCloud Classic 10.
...
To get rid of the database we will move the metadata from the old ownCloud Classic 10 database into dedicated storage providers. This can happen in a user by user fashion. group drives can properly be migrated to group, project or workspaces in this stage.
...
The ownCloud Classic 10 database still holds share information in the `oc_share` and `oc_share_external` tables. They are used to efficiently answer queries about who shared what with whom. In oCIS shares are persisted using a share manager and if desired these grants are also sent to the storage provider so it can set ACLs if possible. Only one system should be responsible for the shares, which in case of treating the storage as the primary source effectively turns the share manager into a cache.

#### User impact
Depending on chosen the share manager provider some sharing requests should be faster: listing incoming and outgoing shares is no longer bound to the ownCloud Classic 10 database but to whatever technology is used by the share provider:
...
4. Shut down ownCloud Classic 10 database
...
The fundamental difference between ownCloud Classic 10 and oCIS is that the file metadata is moved from the database in the `oc_filecache` table (which is misnamed, as it actually is an index) to the storage provider who can place metadata as close to the underlying storage system as possible. In effect, the file metadata is sharded over multiple specialized services.
...
In ownCloud Classic 10 the files are laid out on disk in the *data directory* using the following layout:
```
data
├── einstein
│   ├── cache
...
The *data directory* may also contain subfolders for ownCloud Classic 10 applications like `avatars`, `gallery`, `files_external` and `cache`.
...
In both cases the file metadata, including a full replication of the file tree, is stored in the `oc_filecache` table of an ownCloud Classic 10 database. The primary key of a row is the file id. It is used to attach additional metadata like shares, tags, favorites or arbitrary file properties.
...
2. To search share recipients, both, ownCloud Classic 10 and oCIS need access to the same user directory using e.g. LDAP.
...
The `dn` -> *owncloud internal username* mapping that currently lives in the `oc_ldap_user_mapping` table needs to move into a dedicated `ownclouduuid` attribute in the LDAP server. The idp should send it as a claim so the proxy does not have to look up the user using LDAP again. The username cannot be changed in ownCloud Classic 10 and the oCIS provisioning API will not allow changing it as well. When we introduce the graph api we may allow changing usernames when all clients have moved to that api.

The problem is that the username in ownCloud Classic 10 and in oCIS also need to be the same, which might not be the case when the ldap mapping used a different column. In that case we should add another owncloudusername attribute to the ldap server.
