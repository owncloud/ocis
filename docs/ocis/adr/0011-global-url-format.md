...
When speaking about URLs we have to make a difference between browser URLs and API URLs. Only browser URLs are visible to end users and will be bookmarked. The currently existing and bookmarked ownCloud Classic 10 URLs look something like this:
...
* Existing ownCloud Classic 10 URLs
* ID based URLs
* Path based URLs
* Space based URLs
* Mixed Global URLs
...
The existing ownCloud Classic 10 URLs look like this
...
In contrast to ownCloud Classic 10 path takes precedence and the user is warned when the fileid in his bookmark no longer matches the id on the server: sth. like "The path of the resource has changed, please verify and update your bookmark!"
