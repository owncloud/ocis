...
5. Move to the repo root (`cd ..`)and update the WEB_COMMITID in the `/.drone.env` file to the commit id from the released version (unless the existing commit id is already newer) <!-- DRONE: .drone.env is shared with Drone CI and GitHub Actions; after Drone removal rename file and update all references -->
