...
++~/.ocis/idm/ldap.crt ++
a| [subs=-attributes]
File name of the TLS server certificate for the LDAPS listener. If not definied, the root directory derives from $OCIS_BASE_DATA_PATH:/idm.
...
++~/.ocis/idm/ldap.key ++
a| [subs=-attributes]
File name for the TLS certificate key for the server certificate. If not definied, the root directory derives from $OCIS_BASE_DATA_PATH:/idm.
...
a| [subs=-attributes]
Full path to the IDM backend database. If not definied, the root directory derives from $OCIS_BASE_DATA_PATH:/idm.
