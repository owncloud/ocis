...
a|`USERLOG_EVENTS_TLS_ROOT_CA_CERTIFICATE` +
...
++mem ++
a| [subs=-attributes]
The type of the userlog store. Supported values are: 'mem', 'ocmem', 'etcd', 'redis', 'nats-js', 'noop'. See the text description for details.

a|`USERLOG_STORE_ADDRESSES` +
...
++ ++
a| [subs=-attributes]
(optional) The database name the configured store should use. This has no effect when 'in-memory' stores or 'redis' is configured.
