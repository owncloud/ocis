...
func (a Application) ToExternal(entrypoint string) config.ExternalApp {
	return config.ExternalApp{
		ID:     a.ID,
		Path:   path.Join(entrypoint, a.ID, a.Entrypoint),
		Config: a.Config,
	}
}
...
func List(appsData map[string]config.App, fSystems ...fs.FS) ([]Application, error) {
	...
	return maps.Values(registry), nil
}

func Get(fSystem fs.FS, name string, conf map[string]any) (Application, error) {
	// skip non-directory listings, every app needs to be contained inside a directory
	entry, err := fs.Stat(fSystem, name)
	if err != nil || !entry.IsDir() {
		return Application{}, errors.New("skipping non-app")
	}
	...
	if err != nil {
		...
		return Application{}, errors.Wrap(err, "could not open manifest.json")
	}

	var application Application
	if json.NewDecoder(reader).Decode(&application) != nil {
		// a valid manifest.json is required
		return Application{}, errors.Wrap(err, "could not unmarshal manifest.json")
	}

	if err := validate.Struct(application); err != nil {
		// the application is required to be valid
		return Application{}, errors.Wrap(err, "application is not valid")
	}
