...
func NewService(opts ...Option) (Graph, error) { //nolint:maintidx
		...
		r.Route("/v1beta1", func(r chi.Router) {
			...
			r.Route("/drives", func(r chi.Router) {
				...
				r.Route("/{driveID}", func(r chi.Router) {
					...
					r.Patch("/", svc.UpdateDriveV1Beta1)
					...
					r.Route("/items/{itemID}", func(r chi.Router) {
						...
						r.Route("/permissions", func(r chi.Router) {
							...
							r.Route("/{permissionID}", func(r chi.Router) {
								r.Get("/", driveItemPermissionsApi.GetPermission)
	...
	if options.Config.EnableVaultMode {
		m.Route("/vault/graph", func(r chi.Router) {
			r.Use(autoprop.NewHttpHandler())
			// check the vault permission before the MFA gate
	...
	if options.IdentityBackend == nil {
		...
		case "ldap":
			...
			conn, err := utils.GetLDAPClientFromConfig(&utils.LDAPConn{
				...
				RetryMaxCount:       options.Config.Identity.LDAP.RetryMaxCount,
				RetryBaseDelay:      options.Config.Identity.LDAP.RetryBaseDelay,
				RetryMaxDelay:       options.Config.Identity.LDAP.RetryMaxDelay,
