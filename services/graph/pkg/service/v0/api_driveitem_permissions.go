...
func (s DriveItemPermissionsService) Invite(ctx context.Context, resourceId *storageprovider.ResourceId, invite libregraph.DriveItemInvite) (libregraph.Permission, error) {
	...
	for _, roleID := range invite.GetRoles() {
		...
		if len(allowedResourceActions) == 0 && role.GetId() != unifiedrole.UnifiedRoleDeniedID {
			err = errorcode.New(errorcode.InvalidRequest, "invalid request: role not applicable to this resource")
...
func (s DriveItemPermissionsService) SpaceRootInvite(ctx context.Context, driveID *storageprovider.ResourceId, invite libregraph.DriveItemInvite) (libregraph.Permission, error) {
	...
	gatewayClient, err := s.gatewaySelector.Next()
	if err != nil {
		s.logger.Error().Err(err).Str("resourceId", driveID.GetStorageId()).Msg(err.Error())
		return libregraph.Permission{}, err
	}
...
func (s DriveItemPermissionsService) ListPermissions(ctx context.Context, itemID *storageprovider.ResourceId, listFederatedRoles, selectRoles bool) (libregraph.CollectionOfPermissionsWithAllowedValues, error) {
	...
	if err := errorcode.FromStat(statResponse, err); err != nil {
		s.logger.Warn().Err(err).Str("resourceId", itemID.GetStorageId()).Interface("stat.res", statResponse).Msg("stat failed")
		return collectionOfPermissions, err
	}

	var condition string
	if condition, err = roleConditionForResourceType(statResponse.GetInfo()); err != nil {
		s.logger.Warn().Err(err).Str("resourceId", itemID.GetStorageId()).Interface("condition.res", condition).Msg("role condition failed")
		return collectionOfPermissions, err
	}
	...
	if IsSpaceRoot(statResponse.GetInfo().GetId()) {
		...
		if err != nil {
			s.logger.Warn().Err(err).Str("resourceId", itemID.GetStorageId()).Interface("permissions.res", permissions).Msg("space root permissions error")
			return collectionOfPermissions, err
		}
		...
		if err != nil {
			s.logger.Warn().Err(err).Str("resourceId", itemID.GetStorageId()).Interface("driveItems.res", driveItems).Msg("list user shares error")
			return collectionOfPermissions, err
		}
		if s.config.IncludeOCMSharees {
			...
			if err != nil {
				s.logger.Warn().Err(err).Str("resourceId", itemID.GetStorageId()).Interface("driveItems.res", driveItems).Msg("list ocm shares error")
				return collectionOfPermissions, err
			}
		}
	}
	...
	if err != nil {
		s.logger.Warn().Err(err).Str("resourceId", itemID.GetStorageId()).Interface("driveItems.res", driveItems).Msg("list public shares error")
		return collectionOfPermissions, err
	}
...
func (s DriveItemPermissionsService) ListSpaceRootPermissions(ctx context.Context, driveID *storageprovider.ResourceId) (libregraph.CollectionOfPermissionsWithAllowedValues, error) {
	...
	gatewayClient, err := s.gatewaySelector.Next()
	if err != nil {
		s.logger.Warn().Err(err).Str("resourceId", driveID.GetStorageId()).Msg("gateway error")
		return collectionOfPermissions, err
	}
	...
	if err != nil {
		err = errorcode.FromUtilsStatusCodeError(err)
		s.logger.Warn().Err(err).Str("resourceId", driveID.GetStorageId()).Msg("space permissions error")
		return collectionOfPermissions, err
	}
	...
	if !isSupportedSpaceType {
		err = errorcode.New(errorcode.InvalidRequest, "unsupported space type")
		s.logger.Warn().Err(err).Str("resourceId", driveID.GetStorageId()).Msg(err.Error())
		return collectionOfPermissions, err
	}
...
func (s DriveItemPermissionsService) DeleteSpaceRootPermission(ctx context.Context, driveID *storageprovider.ResourceId, permissionID string) error {
	...
	gatewayClient, err := s.gatewaySelector.Next()
	if err != nil {
		s.logger.Error().Err(err).Str("resourceId", driveID.GetStorageId()).Str("permissionID", permissionID).Msg("gateway error")
		return err
	}
	...
	if err != nil {
		err = errorcode.FromUtilsStatusCodeError(err)
		s.logger.Error().Err(err).Str("resourceId", driveID.GetStorageId()).Str("permissionID", permissionID).Msg("space error")
		return err
	}
...
func (s DriveItemPermissionsService) UpdatePermission(ctx context.Context, itemID *storageprovider.ResourceId, permissionID string, newPermission libregraph.Permission) (libregraph.Permission, error) {
	...
	if err != nil {
		s.logger.Error().Err(err).Str("resourceId", itemID.GetStorageId()).Str("permissionID", permissionID).Str("newPermissionID", *newPermission.Id).Msg("get permissions by id error")
		return libregraph.Permission{}, err
	}
	...
	if !utils.ResourceIDEqual(sharedResourceID, itemID) {
		err = errorcode.New(errorcode.InvalidRequest, "permissionID and itemID do not match")
		s.logger.Error().Err(err).Str("resourceId", itemID.GetStorageId()).Str("permissionID", permissionID).Str("newPermissionID", *newPermission.Id).Msg(err.Error())
		return libregraph.Permission{}, err
	}
	...
	if _, ok := oldPermission.GetLinkOk(); ok {
		...
		if err != nil {
			s.logger.Error().Err(err).Str("resourceId", itemID.GetStorageId()).Str("permissionID", permissionID).Str("newPermissionID", *newPermission.Id).Msg("update public link permission error")
			return libregraph.Permission{}, err
		}
	...
	s.logger.Error().Err(err).Str("resourceId", itemID.GetStorageId()).Str("permissionID", permissionID).Str("newPermissionID", *newPermission.Id).Msg("update permission error")
	return libregraph.Permission{}, err
}
...
func (s DriveItemPermissionsService) UpdateSpaceRootPermission(ctx context.Context, driveID *storageprovider.ResourceId, permissionID string, newPermission libregraph.Permission) (libregraph.Permission, error) {
	...
	gatewayClient, err := s.gatewaySelector.Next()
	if err != nil {
		s.logger.Error().Err(err).Str("resourceId", driveID.GetStorageId()).Str("permissionID", permissionID).Str("newPermissionID", *newPermission.Id).Msg("gateway error")
		return libregraph.Permission{}, err
	}
	...
	if err != nil {
		err = errorcode.FromUtilsStatusCodeError(err)
		s.logger.Error().Err(err).Str("resourceId", driveID.GetStorageId()).Str("permissionID", permissionID).Str("newPermissionID", *newPermission.Id).Msg("space error")
		return libregraph.Permission{}, err
	}

	if space.SpaceType != _spaceTypeProject {
		err = errorcode.New(errorcode.InvalidRequest, "unsupported space type")
		s.logger.Error().Err(err).Str("resourceId", driveID.GetStorageId()).Str("permissionID", permissionID).Str("newPermissionID", *newPermission.Id).Msg(err.Error())
		return libregraph.Permission{}, err
	}
