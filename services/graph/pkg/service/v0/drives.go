...
func (g Graph) disablePersonalSpace(ctx context.Context, userID string) error {
	...
	for _, sp := range lspr.GetStorageSpaces() {
		dRes, derr := client.DeleteStorageSpace(ctx, &storageprovider.DeleteStorageSpaceRequest{Id: &storageprovider.StorageSpaceId{OpaqueId: sp.GetId().GetOpaqueId()}})
		if derr != nil {
			return fmt.Errorf("failed to disable personal space: %w", derr)
		}
		if dRes.GetStatus().GetCode() != cs3rpc.Code_CODE_OK {
			return errorcode.New(cs3StatusToErrCode(dRes.GetStatus().GetCode()), dRes.Status.Message)
		}
	}
	return nil
}

func (g Graph) ensurePersonalSpace(ctx context.Context, userID string) error {
	client, err := g.gatewaySelector.Next()
	if err != nil {
		return err
	}
	...
	if len(lspr.GetStorageSpaces()) > 0 {
		for _, sp := range lspr.GetStorageSpaces() {
			...
			if uRes.GetStatus().GetCode() != cs3rpc.Code_CODE_OK {
				return errorcode.New(cs3StatusToErrCode(uRes.GetStatus().GetCode()), uRes.Status.Message)
			}
		}
	} else {
		user, err := g.identityCache.GetUser(ctx, userID)
		if err != nil {
			return fmt.Errorf("failed to get user: %w", err)
		}
		req := &storageprovider.CreateStorageSpaceRequest{
			Type:  "personal",
			Owner: &userv1beta1.User{Id: &userv1beta1.UserId{OpaqueId: userID}},
		...
		if cRes.GetStatus().GetCode() != cs3rpc.Code_CODE_OK {
			return errorcode.New(cs3StatusToErrCode(cRes.GetStatus().GetCode()), cRes.Status.Message)
		}
	}
	return nil
}
