...
type LDAP struct {
	...
	RetryMaxCount            int           `yaml:"retry_max_count"  env:"OCIS_LDAP_RETRY_MAX_COUNT;GRAPH_LDAP_RETRY_MAX_COUNT"   desc:"Maximum number of retries for failed LDAP operations. Reads retry on transient and network errors; writes retry only when the connection failed before the request was sent (e.g. a reused idle connection found closed), so the request provably never reached the server and cannot be double-applied. Default 1." introductionVersion:"8.2.0"`
	RetryBaseDelay           time.Duration `yaml:"retry_base_delay" env:"OCIS_LDAP_RETRY_BASE_DELAY;GRAPH_LDAP_RETRY_BASE_DELAY" desc:"Base delay for exponential backoff between LDAP retries (e.g. '100ms'). Set to 0 to disable backoff." introductionVersion:"8.2.0"`
	RetryMaxDelay            time.Duration `yaml:"retry_max_delay"  env:"OCIS_LDAP_RETRY_MAX_DELAY;GRAPH_LDAP_RETRY_MAX_DELAY"   desc:"Maximum delay cap for exponential backoff between LDAP retries (e.g. '5s'). 0 (default) uses the library default cap of 60s." introductionVersion:"8.2.0"`
	...
	PoolEnabled         bool          `yaml:"pool_enabled" env:"OCIS_LDAP_POOL_ENABLED;GRAPH_LDAP_POOL_ENABLED" desc:"Enable a bounded pool of LDAP connections instead of a single long-lived reconnecting connection. Concurrent requests then no longer serialize on one connection." introductionVersion:"8.2.0"`
	PoolSize            int           `yaml:"pool_size" env:"OCIS_LDAP_POOL_SIZE;GRAPH_LDAP_POOL_SIZE" desc:"Maximum number of concurrently open LDAP connections when 'OCIS_LDAP_POOL_ENABLED' is 'true'. Defaults to 5 when unset or <= 0." introductionVersion:"8.2.0"`
	...
	GroupAdditionalObjectClasses []string `yaml:"group_additional_objectclasses" env:"OCIS_LDAP_GROUP_ADDITIONAL_OBJECTCLASSES;GRAPH_LDAP_GROUP_ADDITIONAL_OBJECTCLASSES" desc:"Additional object classes to set when creating new groups (e.g. 'posixGroup'). The value of 'GRAPH_LDAP_GROUP_OBJECTCLASS' is always included." introductionVersion:"8.0.2"`
	...
	InstanceMapperCacheTTL         time.Duration `yaml:"instance_mapper_cache_ttl" env:"OCIS_LDAP_INSTANCE_MAPPER_CACHE_TTL" desc:"TTL for the LDAP instance mapper cache (e.g. '60s'). Set to 0 to disable caching of instance mapper lookups. Requires OCIS_MULTI_INSTANCE_ENABLED. See the Environment Variable Types description for more details." introductionVersion:"8.2.0"`
	InstanceMapperCacheCapacity    int           `yaml:"instance_mapper_cache_capacity" env:"OCIS_LDAP_INSTANCE_MAPPER_CACHE_CAPACITY" desc:"Maximum number of entries in the LDAP instance mapper cache (default 10000). The least recently used entry is evicted when the limit is reached. Part of a cache key is the requested instance name, which a client controls, so this bounds the memory a client can make the cache consume. Set to 0 for an unlimited number of entries, which is not recommended for that reason. Requires OCIS_MULTI_INSTANCE_ENABLED." introductionVersion:"8.2.0"`
}
