...
| IDP_TRANSPORT_TLS_CERT | string | ~/.ocis/idp/server.crt | Path/File name of the TLS server certificate (in PEM format) for the IDP service.|
| IDP_TRANSPORT_TLS_KEY | string | ~/.ocis/idp/server.key | Path/File name for the TLS certificate key (in PEM format) for the server certificate to use for the IDP service.|
...
| LDAP_CACERT<br/>IDP_LDAP_TLS_CACERT | string | ~/.ocis/idm/ldap.crt | Path/File name for the root CA certificate (in PEM format) used to validate TLS server certificates of the LDAP service.|
