...
[#deprecation-note-2023-07-11-06-04-11]
[caption=]
...
`WEB_UI_CONFIG` +
xref:deprecation-note-2023-07-11-06-04-11[Deprecation Note]
a| [subs=-attributes]
++string ++
a| [subs=-attributes]
++ ++
...
++true ++
a| [subs=-attributes]
Specifies whether single file link shares should be opened with the default app or not. If not opened by the default app, the Web UI just displays the file details. Defaults to 'true'.

a|`WEB_OPTION_IMPRINT_URL` +
...
a|`WEB_OPTION_ACCESS_DENIED_HELP_URL` +
...
++ ++
a| [subs=-attributes]
Specifies the target URL valid for the ocis instance for the generic logged out / access denied page.
...
++false ++
a| [subs=-attributes]
Specifies whether the access token will be stored in the local storage when set to 'true' or in the session storage when set to 'false''. If stored in the local storage, login state will be persisted across multiple browser tabs, means no additional logins are required. Defaults to 'false'.
