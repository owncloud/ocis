...
[#deprecation-note-2023-10-24-12-14-10]
[caption=]
...
++ ++
a| [subs=-attributes]
The Path prefix where the OCM can be accessed (defaults to /).
...
a|`SHARING_OCM_INVITE_MANAGER_DRIVER` +
...
++json ++
a| [subs=-attributes]
Driver to be used to persist ocm invites. Supported values 'json'.

a|`SHARING_OCM_INVITE_MANAGER_JSON_FILE` +
...
a| [subs=-attributes]
Path to the JSON file where ocm invites data will be stored. If not defined, the root directory derives from $OCIS_BASE_DATA_PATH:/storage.

a|`SHARING_OCM_INVITE_MANAGER_INSECURE` +
...
a|`SHARING_OCM_PROVIDER_AUTHORIZER_PROVIDERS_FILE` +
...
a|`SHARING_OCM_SHARE_PROVIDER_DRIVER` +
...
++json ++
a| [subs=-attributes]
Driver to be used for the ocm share provider. Supported values 'json'.

a|`SHARING_OCM_SHAREPROVIDER_JSON_FILE` +
...
a|`SHARING_OCM_SHARE_PROVIDER_INSECURE` +
...
++json ++
a| [subs=-attributes]
Driver to be used for the ocm core. Supported values 'json'.

a|`SHARING_OCM_CORE_JSON_FILE` +
