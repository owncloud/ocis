...
| OCIS_DISABLE_SSE,FRONTEND_DISABLE_SSE | bool | false | When set to true, clients are informed that the Server-Sent Events endpoint is not accessible.|
...
| FRONTEND_PASSWORD_POLICIES_MIN_LOWER_CASE_CHARACTERS | int | 0 | Define the minimum number of uppercase letters. Defaults to 0 if not set.|
| FRONTEND_PASSWORD_POLICIES_MIN_UPPER_CASE_CHARACTERS | int | 0 | Define the minimum number of lowercase letters. Defaults to 0 if not set.|
