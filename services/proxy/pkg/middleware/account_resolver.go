...
func (m accountResolver) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if ok {
		// we already have a user - use this one
		m.verifyUser(w, req, user)
		return
	}

	claims := oidc.FromContext(ctx)
	if claims == nil {
		// no user and no claims - nothing to resolve
		m.next.ServeHTTP(w, req)
		return
	}

	// resolve user from oidc claims
	m.resolveUserFromClaims(w, req, claims)
}

func (m accountResolver) verifyUser(w http.ResponseWriter, req *http.Request, user *userpb.User) {
...
func (m accountResolver) resolveUserFromClaims(w http.ResponseWriter, req *http.Request, claims map[string]interface{}) {
	ctx := req.Context()

	isMember, isGuest := m.resolveUserType(claims)
	if !isMember && !isGuest {
		m.logger.Info().Interface("claims", claims).Msg("user not part of instance")
...
func (m accountResolver) resolveUserType(claims map[string]interface{}) (isMember bool, isGuest bool) {
	if !m.multiInstance {
	...
	if m.masterID != "" {
		memberClaims := readClaim(m.memberClaim, claims)
		m.logger.Debug().Interface("memberClaims", memberClaims).Str("masterID", m.masterID).Msg("checking master-id")
		for _, miid := range memberClaims {
			if miid != "" && miid == m.masterID {
				m.logger.Debug().Msg("master-id matched!")
				return true, false
			}
		}
	}
	...
	guestClaims := readClaim(m.guestClaim, claims)
	m.logger.Debug().Interface("guestClaims", guestClaims).Msg("checking guest claims")
	for _, giid := range guestClaims {
		if giid != "" && giid == m.instanceID {
			return false, true
		}
	}
	return false, false
...
func readUserIDClaim(path string, claims map[string]interface{}) (string, error) {
	...
	// FIXME: Not sure what this done from here. Does it even work?
