...
func DeleteStaleProcessingNodes(cfg *config.Config) *cli.Command {
	return &cli.Command{
		...
		Action: func(ctx *cli.Context) error {
			...
			for _, spaceID := range spaceIDs {
				deleteStaleUploads(ctx, cfg, spaceID)
			}
...
func globSpaceIDs(cfg *config.Config) []string {
	nodesDir := "nodes"
	spaceIDs := []string{}
	spacesRoot := filepath.Join(cfg.Drivers.OCIS.Root, "spaces")

	// Find all dirs in spacesRoot that have "nodes" inside
	err := filepath.Walk(spacesRoot, func(path string, info os.FileInfo, err error) error {
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error accessing path %s: %v\n", path, err)
			return filepath.SkipDir
		}

		if info.IsDir() && info.Name() == nodesDir {
			// For path i.e. ~/.ocis/storage/users/spaces/9d/408cec-8f0a-4d33-8715-89df1217a10c/nodes
			// spaceID is 9d408cec-8f0a-4d33-8715-89df1217a10c
			spaceID := strings.ReplaceAll(strings.TrimSuffix(strings.TrimPrefix(path, filepath.Join(spacesRoot, "/")), "/"+nodesDir), "/", "")
			spaceIDs = append(spaceIDs, spaceID)
		}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error walking spaces root directory %s: %v\n", spacesRoot, err)
		return nil
	}
...
func deleteStaleUploads(ctx *cli.Context, cfg *config.Config, spaceID string) {
	...
	for _, path := range mpkFiles {
		staleCount += deleteStaleNode(ctx, cfg, path)
	}
...
// returns 1 if the node was deleted, 0 otherwise, for counting purposes
func deleteStaleNode(ctx *cli.Context, cfg *config.Config, path string) int {
	...
	infoExists := !os.IsNotExist(infoStatErr)
	if infoExists {
		return 0
	}
