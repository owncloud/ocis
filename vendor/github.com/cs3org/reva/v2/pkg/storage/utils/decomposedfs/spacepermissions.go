...
func NewPermissions(item PermissionsChecker, selector pool.Selectable[cs3permissions.PermissionsAPIClient]) Permissions {
	return Permissions{item: item, space: selector}
}
...
func (p Permissions) checkPermission(ctx context.Context, perm string, ref *provider.Reference) bool {
	...
	client, err := p.space.Next()
	if err != nil {
		return false
	}
	checkRes, err := client.CheckPermission(ctx, &cs3permissions.CheckPermissionRequest{
		Permission: perm,
