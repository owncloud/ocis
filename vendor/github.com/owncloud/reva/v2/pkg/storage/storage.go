...
type UploadSource struct {
	...
	// ScanResult and ScanDate carry the antivirus verdict the bytes were cleared
	// under, for drivers that expose scan state on the resource. ScanDate is zero
	// when the upload was not scanned; ScanResult is empty for a clean scan, so it
	// cannot be used to tell "clean" from "not scanned".
	ScanResult string
	ScanDate   time.Time
}
