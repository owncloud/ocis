...
func (fs *Decomposedfs) CommitUpload(ctx context.Context, ref *provider.Reference, sessionID string, source storage.UploadSource) error {
	...
	// Recorded on the node, not just the session: the session is deleted once the
	// upload finishes, while ScanData is read back for every stat of the file.
...
func (fs *Decomposedfs) RollbackUpload(ctx context.Context, ref *provider.Reference, sessionID string, nodeExisted bool, sizeDiff int64) error {
	...
	if nodeExisted {
		...
		// The upload created this node, so undoing it means removing it. Purge
		// rather than trash it: the caller is discarding a file that never became
		// visible (a failed virus scan, say), and it must not be recoverable.
		// Purge is also permission-free, which matters because the executant of a
		// write-only share has no Delete permission on what they just uploaded.
