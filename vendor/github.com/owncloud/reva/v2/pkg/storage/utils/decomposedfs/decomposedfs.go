...
var (
	...
	// The upload-related postprocessing events are consumed by the upload
	// coordinator (pkg/upload), which owns the upload lifecycle for every driver.
	// RevertRevision is not part of that lifecycle: the storage-users CLI emits it
	// to undo a revision, and reverting one needs the driver's node internals.
...
func New(o *options.Options, aspects aspects.Aspects, log *zerolog.Logger) (storage.FS, error) {
	...
	// Callers may disable versioning through either the options or the aspects: the
	// posix driver builds its own aspects and only sets it there. Fold it into the
	// options so code reading o.DisableVersioning sees it too.
	...
	// Gated on AsyncFileUploads only because that is the flag that has always
	// enabled this subscription; reverting a revision is otherwise unrelated to
	// async uploads. Widening it would start a consumer in deployments that never
	// had one.
	if o.AsyncFileUploads {
		...
		// A group name is also the JetStream durable consumer name, and members of one
		// group share messages rather than each getting a copy. This must therefore not
		// share the coordinator's group: an upload event delivered here would be
		// dropped by the type filter above and never reach the coordinator.
		ch, err := events.Consume(fs.stream, o.Events.ConsumerGroup+"-revisions", _registeredEvents...)
		if err != nil {
			return nil, err
		}
		// One consumer is enough: revisions are reverted by a maintenance command,
		// not by upload traffic.
...
// ConsumeRevisionEvents handles revision events until ch is closed. Upload
// postprocessing lives in the upload coordinator; this only reverts revisions on
// behalf of the storage-users CLI, which needs the driver's node internals.
func (fs *Decomposedfs) ConsumeRevisionEvents(ch <-chan events.Event) {
	log := logger.New()
	for event := range ch {
		fs.processEvent(context.Background(), event, log)
	}
}
