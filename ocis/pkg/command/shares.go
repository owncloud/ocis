...
func cleanOrphanedGrants(c *cli.Context, cfg *config.Config) error {
	...
	// selector to query share-manager via collaboration API
	shareSelector, err := pool.SharingCollaborationSelector(cfg.Sharing.Reva.Address, revaOpts...)
	if err != nil {
		return configlog.ReturnError(err)
	}
	...
	for idx, space := range spaces {
		...
		err = walker.Walk(serviceUserCtx, space.GetRoot(), func(wd string, info *provider.ResourceInfo, walkErr error) error {
			...
			shares, shareErr := listSharesForResource(serviceUserCtx, shareSelector, info.GetId())
			if shareErr != nil {
			...
			for _, g := range grants {
				if !hasMatchingShareForGrant(shares, g) {
					orphanCandidates++
					if !dryRun && shares != nil && len(shares) >= 0 {
...
func listSharesForResource(ctx context.Context, shareSelector pool.Selectable[collaboration.CollaborationAPIClient], resourceID *provider.ResourceId) ([]*collaboration.Share, error) {
