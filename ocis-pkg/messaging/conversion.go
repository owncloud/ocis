...
func NewConverter(loc string, gwc gateway.GatewayAPIClient, machineAuthAPIKey string, name string, filesystem fs.FS) *Converter {
	return &Converter{
		locale:            loc,
		gwClient:          gwc,
...
func (c *Converter) ConvertEvent(id string, event interface{}) (OC10Notification, error) {
	...
	case events.PostprocessingStepFinished:
		...
		case events.PPStepAntivirus:
			...
			return c.virusMessage(id, VirusFound, ev.ExecutingUser, res.ResourceID, ev.Filename, res.Description, res.Scandate)
		case events.PPStepPolicies:
			return c.policiesMessage(id, PoliciesEnforced, ev.ExecutingUser, ev.Filename, time.Now())
		default:
			return OC10Notification{}, fmt.Errorf("unknown postprocessing step: %s", ev.FinishedStep)
		}
	// space related
	case events.SpaceDisabled:
		return c.spaceMessage(id, SpaceDisabled, ev.Executant, ev.ID.GetOpaqueId(), ev.Timestamp)
	case events.SpaceDeleted:
		return c.spaceDeletedMessage(id, ev.Executant, ev.ID.GetOpaqueId(), ev.SpaceName, ev.Timestamp)
	case events.SpaceShared:
		return c.spaceMessage(id, SpaceShared, ev.Executant, ev.ID.GetOpaqueId(), ev.Timestamp)
	case events.SpaceUnshared:
		return c.spaceMessage(id, SpaceUnshared, ev.Executant, ev.ID.GetOpaqueId(), ev.Timestamp)
	case events.SpaceMembershipExpired:
		return c.spaceMessage(id, SpaceMembershipExpired, ev.SpaceOwner, ev.SpaceID.GetOpaqueId(), ev.ExpiredAt)

	// share related
	case events.ShareCreated:
		return c.shareMessage(id, ShareCreated, ev.Executant, ev.ItemID, ev.ShareID, utils.TSToTime(ev.CTime))
	case events.ShareExpired:
		return c.shareMessage(id, ShareExpired, ev.ShareOwner, ev.ItemID, ev.ShareID, ev.ExpiredAt)
	case events.ShareRemoved:
		return c.shareMessage(id, ShareRemoved, ev.Executant, ev.ItemID, ev.ShareID, ev.Timestamp)
	}
}
...
func (c *Converter) spaceDeletedMessage(eventid string, executant *user.UserId, spaceid string, spacename string, ts time.Time) (OC10Notification, error) {
	...
	subj, subjraw, msg, msgraw, err := composeMessage(SpaceDeleted, c.locale, c.fs, map[string]interface{}{
...
func (c *Converter) spaceMessage(eventid string, nt NotificationTemplate, executant *user.UserId, spaceid string, ts time.Time) (OC10Notification, error) {
	...
	subj, subjraw, msg, msgraw, err := composeMessage(nt, c.locale, c.fs, map[string]interface{}{
		"username":  usr.GetDisplayName(),
...
func composeMessage(nt NotificationTemplate, locale string, filesystem fs.FS, vars map[string]interface{}) (string, string, string, string, error) {
	subjectraw, messageraw := loadTemplates(nt, locale, filesystem)
...
func loadTemplates(nt NotificationTemplate, locale string, filesystem fs.FS) (string, string) {
